...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"  xmlns:arch="http://archive.ru">
	...
	<xsl:template name="output-attributes">
		...
		<xsl:choose>
			<xsl:when test="string-length($InvDocCode-default)=0 and string-length($InvDocName-default)=0 and string-length($InvDocNumber)=0 and string-length($InvDocDate)=0">
			</xsl:when>
			<xsl:otherwise>
										...
										<xsl:choose>
											<xsl:when test="0 = count($InvDocCode)">
												<xsl:message terminate="yes">
													<xsl:text>Необходимо заполнить в Описи код документа либо в Опись добавить документ </xsl:text>
											...
											<xsl:when test="$InvDocCode != $InvDocCode">
												<xsl:message terminate="yes">
													...
													<xsl:text>В пакете и в Описи несколько одинаковых документов (</xsl:text>
													<xsl:value-of select="count($InvDocCode)"/>
													<xsl:value-of select="$doc-num-human"/>
												</xsl:message>
											</xsl:when>
	...
	<xsl:key name="InvDocCode" use="concat(../*[ local-name() = 'InvDocCode' ], ../*[ local-name() = 'Note' ],  ../*[ local-name() = 'InvDocNumber' ],  ../*[ local-name() = 'InvDocDate' ])" match="*[local-name()='WHDocInventory' and @DocumentModeID='1008014E']/*[local-name()='InventDocument']/*[local-name()='InvDocCode'][string()]"/>
	<xsl:key name="InvDocCode" use="concat(../*[ local-name() = 'InvDocCode' ], ../*[ local-name() = 'InvDocNumber' ], ../*[ local-name() = 'InvDocDate' ] )" match="*[local-name()='WHDocInventory' and @DocumentModeID='1008014E']/*[local-name()='InventDocument']/*[local-name()='InvDocCode'][string()]"/>
	<xsl:key name="InvDocCode" use="concat(../*[ local-name() = 'Note' ], ../*[ local-name() = 'InvDocNumber' ], ../*[ local-name() = 'InvDocDate' ])" match="*[local-name()='WHDocInventory' and @DocumentModeID='1008014E']/*[local-name()='InventDocument']/*[local-name()='InvDocCode'][string()]"/>
	<xsl:key name="InvDocCode" use="concat(../*[ local-name() = 'InvDocNumber' ], ../*[ local-name() = 'InvDocDate' ] )" match="*[local-name()='WHDocInventory' and @DocumentModeID='1008014E']/*[local-name()='InventDocument']/*[local-name()='InvDocCode'][string()]"/>


	<xsl:variable name="Inventory" select="//*[local-name()='WHDocInventory' and @DocumentModeID='1008014E']"/>
	<xsl:variable name="Inventory-count" select="count($Inventory)"/>
	<xsl:template match="//*[*/*[local-name()='WHDocInventory' and @DocumentModeID='1008014E']]">
		<!--выбрасываем опись (вместе с её ContainerDoc/DocBody). Она нужна только чтобы взять из неё InvDocCode-->
	</xsl:template>
</xsl:stylesheet>
